...
In addition to the cycles counter, we collect the number of instructions retired, cache misses, and OS context switches. Accessing these counters is done through a library we wrote called \textit{libpmu}\footnote{Source code can be found at \url{https://github.com/perf-toolbox/libpmu}}. Designed as a cross-platform tool, \textit{libpmu} interfaces with Performance Monitoring Units (PMUs) and even extends support to some unconventional architectures, such as SiFive's U74 core.
...
For the collected dataset, we use the benchmarking tool to measure basic block latency. We use AMD Ryzen 5 3600 CPU for measurements.
We also capture basic block throughput estimations from the \textit{llvm-mca} tool. Figure \ref{pic:cov} shows the distribution of the coefficient
of variation for the measured data. Figure \ref{pic:lift_chart} shows the lift chart for measured throughput values compared to \textit{llvm-mca}
estimations. The mean squared error value for this experiment was $16.8$.

An example of mispredicted basic blocks can be found in appendix \ref{appendix:listings}. These examples show that \textit{llvm-mca}
particularly struggles with stack instruction optimizations happening inside a processor core.
...
\begin{figure}[h]
  ...
  \centering
  \label{pic:lift_chart}
  \includegraphics[width=0.9\columnwidth]{lift_chart_35_cycles}
\end{figure}

Finally, we build a graph representation for each basic block in the dataset. Figure \ref{pic:sample_graph} shows
an example of such a graph for a basic block snippet below.
